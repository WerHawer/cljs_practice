...
* [`src/clj/test_proj/`](src/clj/test_proj/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/test_proj/`](src/cljs/test_proj/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/test_proj/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `test-proj` project root directory
...
in the [`src/clj/test_proj/`](src/clj/test_proj/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/test_proj/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns test-proj.example
  (:require [test-proj.config :as config])
